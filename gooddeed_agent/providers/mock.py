...
# What the stub says for each free-text field the agent asks for. This text
# reaches real users -- on a score, a rejected report, an org summary -- so it
# reads like an ordinary verdict rather than announcing itself as fake. (It used
# to prefix "[mock]", which showed up on public cards.) That a stub is running
# is reported where it belongs: /health says "llm_provider": "mock", the app
# shows a demo-scoring banner, and the server logs it at startup.
_MOCK_TEXT: dict[bool, dict[str, str]] = {
    False: {
        "rationale": "This looks like a genuine submission that matches the description.",
        "reason": "This looks like a real, fixable problem in a public place.",
        "summary": "A community organization with a public presence and regular activity.",
        "cause_summary": "A charitable cause with a public donation page.",
        "organization": "Community organization",
        "*": "Checked automatically.",
    },
    True: {
        "rationale": "We couldn't confirm this matches the description. A clearer photo and a few specific details would help.",
        "reason": "We couldn't tell whether this is a fixable problem. Try a clearer photo and a short description.",
        "summary": "We couldn't find enough public information to confirm this organization.",
        "cause_summary": "We couldn't confirm what this page is for.",
        "organization": "",
...
class MockLLMProvider:
    ...
    def _value_for(
        ...
        if kind == "array":
            return []
        # string
        table = _MOCK_TEXT[looks_bad]
        return table[key] if key in table else table["*"]  # `in`, not `or`: "" is a valid answer
